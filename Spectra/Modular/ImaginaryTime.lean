...
theorem IsKMSState.imaginaryTime {ω : State A} {α : Dynamics A} {β : ℝ} (hβ : 0 < β)
  ...
  obtain ⟨F⟩ := hkms a b
  -- Bundle `ω` as a continuous linear functional.
  set ωL : A →L[ℂ] ℂ := ⟨ω.toFun, ω.continuous⟩ with hωL
  -- The analytic continuation of the correlation: `G z = ω (a · σ_z b)`.
  ...
  have hGdiff : Differentiable ℂ G :=
    ωL.differentiable.comp (hσdiff.const_mul a)
  -- `G` is bounded on the closed strip: `‖σ_z b‖ = ‖σ_{i·Im z} b‖`, bounded over `Im z ∈ [0,β]`.
  have hGbdd : BddAbove (norm '' (G '' ClosedStrip β)) := by
    ...
    show ‖G z‖ ≤ ‖ωL‖ * (‖a‖ * M)
    simp only [hG_def, Dynamics.sigma]
