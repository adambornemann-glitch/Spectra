...
Filename: BochnerTheorem/Continuity.lean
-/
...
lemma pd_oscillation_sq (hf : PositiveDefinite f) (hH : IsHermitian f) (s t : ℝ) :
  ...
  -- Nuclear simp: extract .re from every complex product
  simp only [
    map_sub, Complex.add_re, Complex.sub_re, Complex.mul_re, Complex.neg_re,
    Complex.sub_im, Complex.mul_im, Complex.neg_im, Complex.conj_re, Complex.conj_im,
    Complex.ofReal_re, Complex.ofReal_im, hw_def, Complex.sub_re, Complex.sub_im,
    mul_zero, zero_mul, sub_zero, add_zero, neg_zero, zero_add] at hpd
  -- ‖w‖² in components
  ...
  -- Prove the factored form equals the expanded PD sum
  -- The factored form now genuinely equals the PD sum
  convert hpd using 1
  simp only [← hp_def]
  ring_nf
