...
Filename: SpectralTheory/Herglotz/FejerMeans.lean
-/
...
def fejerMeanDensity (ψ : H) (N : ℕ) (θ : ℝ) : ℂ :=
  ∑ n ∈ Finset.Icc (-(N : ℤ)) N,
...
/-- The Fejér mean density is non-negative (real part ≥ 0, imaginary part = 0).

This follows from positive definiteness: the density can be written as
`|∑_{k=0}^{N} α_k e^{-ikθ}|²` for suitable `α_k`, which uses the
factorization of the Fejér kernel as `|D_N(θ)|² / (N+1)` where `D_N`
is the Dirichlet kernel. -/
lemma fejerMeanDensity_nonneg (hU : Cayley.Unitary U) (ψ : H) (N : ℕ) (θ : ℝ) :
