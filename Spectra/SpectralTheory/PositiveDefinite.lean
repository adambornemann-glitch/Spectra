...
Filename: SpectralTheory/PositiveDefinite.lean
-/
...
lemma pd_at_zero_nonneg (hf : PositiveDefinite f) : 0 ≤ (f 0).re := by
...
lemma pd_two_point_add (hf : PositiveDefinite f) (t : ℝ) :
...
Gives: `0 ≤ 2·(f 0).re - Re(f(t) + f(-t))` -/
lemma pd_two_point_sub (hf : PositiveDefinite f) (t : ℝ) :
    0 ≤ 2 * (f 0).re - (f t + f (-t)).re := by
  have h := hf 2 ![0, t] ![1, -1]
  simp only [Fin.sum_univ_two, Matrix.cons_val_zero, Matrix.cons_val_one] at h
...
lemma pd_two_point_I (hf : PositiveDefinite f) (t : ℝ) :
...
The sum evaluates to `2·f(0) - 2·Im(f(t))` under Hermitian symmetry,
so the PD condition yields `Im(f(t)) ≤ (f 0).re`. -/
lemma pd_two_point_neg_I (hf : PositiveDefinite f) (t : ℝ) :
    0 ≤ (f 0 + -(I * f (-t)) + (I * f t + f 0)).re := by
  have h := hf 2 ![0, t] ![1, -I]
  simp only [Fin.sum_univ_two, Matrix.cons_val_zero, Matrix.cons_val_one] at h
  simp only [map_one, map_neg, one_mul, neg_mul, sub_self, sub_zero, zero_sub] at h
...
lemma pd_hermitian_at_zero (hf : PositiveDefinite f) (hH : IsHermitian f) :
    f 0 = ↑((f 0).re) ∧ 0 ≤ (f 0).re :=
...
lemma pd_hermitian_re_le (hf : PositiveDefinite f) (hH : IsHermitian f) (t : ℝ) :
    (f t).re ≤ (f 0).re := by
...
lemma pd_hermitian_re_neg_le (hf : PositiveDefinite f) (hH : IsHermitian f) (t : ℝ) :
    -(f 0).re ≤ (f t).re := by
...
lemma pd_hermitian_re_abs_le (hf : PositiveDefinite f) (hH : IsHermitian f) (t : ℝ) :
    |(f t).re| ≤ (f 0).re :=
...
lemma pd_hermitian_im_le (hf : PositiveDefinite f) (hH : IsHermitian f) (t : ℝ) :
    (f t).im ≤ (f 0).re := by
...
lemma pd_hermitian_im_neg_le (hf : PositiveDefinite f) (hH : IsHermitian f) (t : ℝ) :
    -(f 0).re ≤ (f t).im := by
...
lemma pd_hermitian_im_abs_le (hf : PositiveDefinite f) (hH : IsHermitian f) (t : ℝ) :
    |(f t).im| ≤ (f 0).re :=
...
lemma pd_hermitian_norm_bound (hf : PositiveDefinite f) (hH : IsHermitian f) (t : ℝ) :
    ‖f t‖ ≤ (f 0).re := by
  by_cases hft : f t = 0
...
lemma pd_hermitian_norm_sq_bound (hf : PositiveDefinite f) (hH : IsHermitian f) (t : ℝ) :
    ‖f t‖ ^ 2 ≤ (f 0).re ^ 2 :=
...
lemma pdVariance_nonneg (hf : PositiveDefinite f) (hH : IsHermitian f) (h : ℝ) :
    0 ≤ pdVariance f h :=
...
lemma pdVariance_le (hf : PositiveDefinite f) (hH : IsHermitian f) (h : ℝ) :
...
lemma pdVariance_tendsto_zero (_hf : PositiveDefinite f) (_hH : IsHermitian f)
