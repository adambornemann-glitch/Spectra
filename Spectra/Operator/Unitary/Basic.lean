...
lemma normal_bounded_below_isUnit [Nontrivial H] {T : H →L[ℂ] H}
  ...
  have h_ker : LinearMap.ker (↑T : H →ₗ[ℂ] H) = ⊥ := by
    rw [LinearMap.ker_eq_bot]; intro x y h; exact h_inj h
  have h_range : LinearMap.range (↑T : H →ₗ[ℂ] H) = ⊤ := by
    rw [LinearMap.range_eq_top]; intro y; exact h_surj y
    ...
    · have _hφ_norm_pos : ‖φ‖ > 0 := norm_pos_iff.mpr hφ
      ...
      calc ‖(U - w • ContinuousLinearMap.id ℂ H) φ‖
        ...
        _ = ‖φ‖ * ‖(U - w • ContinuousLinearMap.id ℂ H) (‖φ‖⁻¹ • φ)‖ := by
            ...
            congr 1
            have : (‖φ‖⁻¹ : ℝ) • φ = ((↑‖φ‖⁻¹ : ℂ) • φ) := by
              exact Eq.symm (coe_smul ‖φ‖⁻¹ φ)
            rw [smul_sub, this, map_smul, coe_smul, smul_comm]
            rfl
