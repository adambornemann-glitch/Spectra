...
* the configuration space `R3 := EuclideanSpace ℝ (Fin 3)` with its Lebesgue
  measure structure;
* the Hilbert space `l2R3 := Lp ℂ 2 (volume : Measure R3)`;
...
/-- `R3`'s measurable structure is the Borel σ-algebra generated by its metric
    topology. -/
instance : MeasurableSpace R3 := borel R3

/-- Immediate from the `MeasurableSpace R3 := borel R3` instance above: the
    `BorelSpace` class asserts that the measurable space *is* `borel R3`, so
    with that definitional choice the two sides are syntactically equal and
    `⟨rfl⟩` discharges it. -/
...
abbrev l2R3 : Type := Lp ℂ 2 (volume : Measure R3)
...
def HasWeakDerivative (f : l2R3) (i : Fin 3) (g : l2R3) : Prop :=
