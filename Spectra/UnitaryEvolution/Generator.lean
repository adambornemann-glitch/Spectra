...
/-- A strongly continuous one-parameter unitary group.  Note that inner-product
preservation only makes each `U t` an *isometry*; it is the group law that supplies the
inverse `U (-t)`, upgrading isometry to genuine unitarity (see `inverse_eq_adjoint`). -/
