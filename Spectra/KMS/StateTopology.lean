...
With `stateSet_isCompact` (weak-* compact), `stateSet_convex`, and nonemptiness, **Krein–Milman**
(`IsCompact.extremePoints_nonempty`) yields the existence of *pure states* — the extreme points
of the state space:

```
example (hne : (stateSet A).Nonempty) : (Set.extremePoints ℝ (stateSet A)).Nonempty :=
  IsCompact.extremePoints_nonempty stateSet_isCompact hne
```

This is one lemma call away, but it currently does not elaborate: `IsCompact.extremePoints_nonempty`
needs the **real** locally-convex structure `[ContinuousSMul ℝ (WeakDual ℂ A)]` and
`[LocallyConvexSpace ℝ (WeakDual ℂ A)]`, which Mathlib does not register for a **complex** weak
dual (and the real module structure it would use does not defeq-match the one Krein–Milman infers
— an instance-diamond gap, not a mathematical one). Supplying a coherent real-LCTVS instance on
`WeakDual ℂ A` (or upstreaming it to Mathlib) immediately discharges pure-state existence. -/
