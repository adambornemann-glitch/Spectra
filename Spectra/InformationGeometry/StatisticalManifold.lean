...
noncomputable def fisherMetric : RiemannianMetric n :=
...
lemma fisherMetric_pos_def {θ : ParamSpace n}
...
lemma fisherInnerProduct_eq_bilin {θ : ParamSpace n}
    (hθ : θ ∈ S.domain)
    (v w : ParamSpace n) :
...
lemma fisherInnerProduct_eq_expectation {θ : ParamSpace n}
    (hθ : θ ∈ S.domain)
    (v w : ParamSpace n) :
    S.fisherInnerProduct hθ v w =
...
lemma fisherInnerProduct_eq_sum {θ : ParamSpace n}
    (hθ : θ ∈ S.domain)
    (v w : ParamSpace n) :
    S.fisherInnerProduct hθ v w =
      ∑ i : Fin n, ∑ j : Fin n,
...
lemma fisherInnerProduct_symm {θ : ParamSpace n}
    (hθ : θ ∈ S.domain)
    (v w : ParamSpace n) :
    S.fisherInnerProduct hθ v w =
...
lemma fisherInnerProduct_pos {θ : ParamSpace n}
    (hθ : θ ∈ S.domain)
...
lemma fisherInnerProduct_nonneg {θ : ParamSpace n}
    (hθ : θ ∈ S.domain)
    (v : ParamSpace n) :
    0 ≤ S.fisherInnerProduct hθ v v :=
...
/-- The **Fisher norm** of a tangent vector at `θ`:
  `‖v‖_θ = √(g_θ(v, v))`. -/
noncomputable def fisherNorm {θ : ParamSpace n}
    (hθ : θ ∈ S.domain) (v : ParamSpace n) : ℝ :=
  Real.sqrt (S.fisherInnerProduct hθ v v)

/-- The Fisher norm is nonneg. -/
lemma fisherNorm_nonneg {θ : ParamSpace n}
    (hθ : θ ∈ S.domain) (v : ParamSpace n) :
    0 ≤ S.fisherNorm hθ v :=
  Real.sqrt_nonneg _

/-- `‖v‖_θ = 0 ⟺ v = 0`. -/
lemma fisherNorm_eq_zero_iff {θ : ParamSpace n}
    (hθ : θ ∈ S.domain) (v : ParamSpace n) :
...
lemma fisherNorm_pos {θ : ParamSpace n}
    (hθ : θ ∈ S.domain)
    {v : ParamSpace n} (hv : v ≠ 0) :
    0 < S.fisherNorm hθ v :=
...
direction `δθ`. -/
lemma infinitesimally_identifiable {θ : ParamSpace n}
