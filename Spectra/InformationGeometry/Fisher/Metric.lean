...
lemma fisherBilin_add_left {θ : ParamSpace n}
    (hθ : θ ∈ M.paramDomain)
...
lemma fisherBilin_smul_left {θ : ParamSpace n}
    (hθ : θ ∈ M.paramDomain)
    (hSq : M.ScoreSqIntegrableModel θ)
...
lemma fisherBilin_add_right {θ : ParamSpace n}
    (hθ : θ ∈ M.paramDomain)
    (hSq : M.ScoreSqIntegrableModel θ)
    (v w₁ w₂ : ParamSpace n) :
...
lemma fisherBilin_smul_right {θ : ParamSpace n}
    (hθ : θ ∈ M.paramDomain)
    (hSq : M.ScoreSqIntegrableModel θ)
    (c : ℝ) (v w : ParamSpace n) :
...
def fisherBilinForm {θ : ParamSpace n}
  ...
  LinearMap.mk₂ ℝ (M.fisherBilin θ)
    ...
    (fun c v w => by
      rw [M.fisherBilin_smul_left hθ hSq]; ring_nf)
    (M.fisherBilin_add_right hθ hSq)
    (fun c v w => by
      rw [M.fisherBilin_smul_right hθ hSq]; ring_nf)

/-- The bilinear form evaluates to the raw `fisherBilin`. -/
@[simp]
lemma fisherBilinForm_apply {θ : ParamSpace n}
    (hθ : θ ∈ M.paramDomain)
    (hSq : M.ScoreSqIntegrableModel θ)
    (v w : ParamSpace n) :
...
lemma fisherBilinForm_symm {θ : ParamSpace n}
    (hθ : θ ∈ M.paramDomain)
    (hSq : M.ScoreSqIntegrableModel θ)
    (v w : ParamSpace n) :
...
lemma fisherBilinForm_nonneg {θ : ParamSpace n}
    (hθ : θ ∈ M.paramDomain)
    (hSq : M.ScoreSqIntegrableModel θ)
    (v : ParamSpace n) :
...
lemma fisherBilinForm_pos_def {θ : ParamSpace n}
    (hθ : θ ∈ M.paramDomain)
    (hSq : M.ScoreSqIntegrableModel θ)
...
lemma fisherBilinForm_pos {θ : ParamSpace n}
    (hθ : θ ∈ M.paramDomain)
    (hSq : M.ScoreSqIntegrableModel θ)
    (hInj : M.ScoreInjective θ)
    (hInt : ∀ w : ParamSpace n, Integrable
...
lemma eval_symm {θ : ParamSpace n} (hθ : θ ∈ g.domain)
    (v w : ParamSpace n) :
...
lemma eval_pos {θ : ParamSpace n} (hθ : θ ∈ g.domain)
    {v : ParamSpace n} (hv : v ≠ 0) :
    0 < g.eval θ v v :=
  g.metricMatrix_pos_def θ hθ v hv

/-- The metric evaluation is nonneg. -/
lemma eval_nonneg {θ : ParamSpace n} (hθ : θ ∈ g.domain)
    (v : ParamSpace n) :
    0 ≤ g.eval θ v v := by
...
@[simp]
lemma fisherRiemannianMetric_matrix (hF : M.SmoothFisherModel)
...
lemma fisherRiemannianMetric_eval (hF : M.SmoothFisherModel)
...
lemma fisherMatrix_continuousOn (hF : M.SmoothFisherModel)
    (i j : Fin n) :
...
lemma fisherBilin_continuousOn (hF : M.SmoothFisherModel)
    (v w : ParamSpace n) :
