...
lemma preserves_fisher {θ : ParamSpace n} (hθ : θ ∈ M.paramDomain)
  ...
  have h_RHS : fderiv ℝ (fun θ₂ => fderiv ℝ (M.klDiv θ) θ₂
    ...
    have h_diff : DifferentiableAt ℝ (fun θ₂ =>
      ...
      obtain ⟨g₁, hg₁, _⟩ := M.cross_score_hasFDerivAt hθ b
      exact (hg₁.congr_of_eventuallyEq (by
        filter_upwards [M.isOpen_paramDomain.mem_nhds hθ] with θ' hθ'
        exact M.klDiv_partial_j hθ hθ' b)).differentiableAt
  ...
  have h_LHS : fderiv ℝ (fun θ₂ =>
      ...
      have h_chain_ev : ∀ᶠ θ₂ in 𝓝 θ,
        ...
        have hKL_diff : ∀ θ₀ ∈ M.paramDomain, DifferentiableAt ℝ (M.klDiv α) θ₀ := by
          intro θ₀ hθ₀
        ...
        have hcomp := ((hKL_diff _ hθ₂).hasFDerivAt.comp θ₂
      ...
      set v₀ := fderiv ℝ (F.φ t) θ (EuclideanSpace.single b 1) with hv₀_def
      -- ── The "live" function G and "frozen" function R ──
      set G : ParamSpace n → ℝ := fun θ₂ =>
        fderiv ℝ (M.klDiv α) (F.φ t θ₂)
          (fderiv ℝ (F.φ t) θ₂ (EuclideanSpace.single b 1)) with hG_def
      ...
      -- Part II: HasFDerivAt for R(θ₂) = Σⱼ v₀ⱼ · Sⱼ(θ₂)
      ...
      have hGR_eq : ∀ θ₂, G θ₂ - R θ₂ =
          fderiv ℝ (M.klDiv α) (F.φ t θ₂)
            (fderiv ℝ (F.φ t) θ₂ (EuclideanSpace.single b 1) - v₀) := by
      ...
      have h_factor1 : Tendsto
        ...
        have h_comp : ∀ j : Fin n, Tendsto
          ...
          have h_ev : (fun θ' => fderiv ℝ (M.klDiv α) θ'
            ...
            filter_upwards [M.isOpen_paramDomain.mem_nhds hα] with θ' hθ'
            exact klDiv_partial_j M hα hθ' j
          obtain ⟨_, hg_j, _⟩ := cross_score_hasFDerivAt M hα j
      ...
      have h_factor2 : ∃ C > 0, ∀ᶠ θ₂ in 𝓝 θ,
          ‖fderiv ℝ (F.φ t) θ₂ (EuclideanSpace.single b 1) - v₀‖ ≤
          C * ‖θ₂ - θ‖ := by
        have h_diff : DifferentiableAt ℝ (fun θ₂ =>
            fderiv ℝ (F.φ t) θ₂ (EuclideanSpace.single b 1)) θ := by
          have h1 : ContDiff ℝ ⊤ (fderiv ℝ (F.φ t)) :=
            hφ_smooth.fderiv_right (by exact Std.IsPreorder.le_refl (⊤ + 1))
          exact (h1.differentiable (by simp)).differentiableAt.clm_apply
            (differentiableAt_const _)
        set L := fderiv ℝ (fun θ₂ =>
            fderiv ℝ (F.φ t) θ₂ (EuclideanSpace.single b 1)) θ
        have hfd := h_diff.hasFDerivAt
        ...
        calc ‖fderiv ℝ (F.φ t) θ₂ (EuclideanSpace.single b 1) - v₀‖
            = ‖(fderiv ℝ (F.φ t) θ₂ (EuclideanSpace.single b 1) - v₀ -
                L (θ₂ - θ)) + L (θ₂ - θ)‖ := by abel_nf
          _ ≤ ‖fderiv ℝ (F.φ t) θ₂ (EuclideanSpace.single b 1) - v₀ -
                L (θ₂ - θ)‖ + ‖L (θ₂ - θ)‖ := norm_add_le _ _
          _ ≤ 1 * ‖θ₂ - θ‖ + ‖L‖ * ‖θ₂ - θ‖ := by
              apply add_le_add
              · have hest := hball hθ₂
                exact hest
      ...
      have hGR_littleo : (fun θ₂ => G θ₂ - R θ₂) =o[𝓝 θ] (fun θ₂ => θ₂ - θ) := by
        ...
        rw [hGR_eq]
        calc ‖fderiv ℝ (M.klDiv α) (F.φ t θ₂)
                (fderiv ℝ (F.φ t) θ₂ (EuclideanSpace.single b 1) - v₀)‖
            ≤ ‖fderiv ℝ (M.klDiv α) (F.φ t θ₂)‖ *
              ‖fderiv ℝ (F.φ t) θ₂ (EuclideanSpace.single b 1) - v₀‖ :=
          ...
          _ ≤ (ε / C) * (C * ‖θ₂ - θ‖) := by
              ...
              · rw [dist_eq_norm, sub_zero, norm_norm] at h1
                exact le_of_lt h1
              · exact h2
              · exact norm_nonneg ((fderiv ℝ (F.φ t) θ₂) (EuclideanSpace.single b 1) - v₀)
