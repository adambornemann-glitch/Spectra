...
lemma sum_normSq_mulVec_eq_of_unitary {n : ℕ} (U : Matrix (Fin n) (Fin n) ℂ)
  ...
  have h2 : Uᴴ * U = 1 := mul_eq_one_comm.mpr hU
...
lemma nsmul_one_mulVec {m : ℕ} (k : ℕ) (x : Fin m → ℂ) :
...
lemma star_dotProduct_nsmul {m : ℕ} (k : ℕ) (x : Fin m → ℂ) :
...
lemma unitary_add_conjTranspose_ge_neg_two {n : ℕ} [NeZero n]
  ...
  -- Goal: 0 ≤ 2 * (star x ⬝ᵥ x).re + 2 * (star x ⬝ᵥ U *ᵥ x).re
  have h_self_eq : (star x ⬝ᵥ x).re = ∑ i, ‖x i‖^2 := by
    have := star_dotProduct_self_eq_sum_normSq x
    exact congrArg Complex.re this
  rw [h_self_eq]
...
lemma norm_unitary_add_conjTranspose_mulVec {n : ℕ} [NeZero n]
  ...
  have hUH_U : Uᴴ * U = 1 := mul_eq_one_comm.mpr hU
  ...
  have h_norm_x : ∑ i, ‖x i‖^2 = ‖x'‖^2 := by rw [EuclideanSpace.norm_sq_eq]
  have h_norm_Ux : ‖Ux'‖^2 = ∑ i, ‖(U.mulVec x) i‖^2 := by rw [EuclideanSpace.norm_sq_eq]
  have h_norm_UHx : ‖UHx'‖^2 = ∑ i, ‖(Uᴴ.mulVec x) i‖^2 := by rw [EuclideanSpace.norm_sq_eq]
  rw [h_norm_sq, h_add]
