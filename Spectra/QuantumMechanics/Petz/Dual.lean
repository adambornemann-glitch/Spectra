...
The Petz recovery map `petzMap Φ σ τ` (in `RecoveryMap.lean`) takes the Heisenberg dual `Φ = N†` as an
abstract *unital* map. This file makes that dual a genuine object rather than a hypothesis.
...
(`IsHeisenbergDual.unital_iff_tracePreserving`): the forward direction is a one-line consequence of the
duality relation at `B = 1`; the converse uses **separation of the trace pairing**
...
trace duality `B(H) = (T(H))*` (the "predual" theorem, an open Trace-Class Hard Core milestone) — that
turns `Nd` from a *given* map satisfying `IsHeisenbergDual` into one produced *from* `N`. The
...
`ρ ↦ K · ρ · K†`. It is unital exactly when `K` is an isometry (`K† K = 1`), and then it is a genuine
Heisenberg-picture channel for which the Petz map recovers the reference with **no hypothesis on the
dual** — `petzMap_adjointConj_recovery`.
...
/-- `Nd : B(H) → B(H)` is a **Heisenberg dual** of the channel `N : T(H) → T(H)` when it satisfies the
trace-duality relation `tr(N(ρ) · B) = tr(ρ · Nd(B))` for all trace-class `ρ` and bounded `B`. -/
...
duality relation and using `Nd 1 = 1` gives `tr(N ρ) = tr ρ`. (This is the direction the Petz recovery
theorem consumes: unitality of `N†` is exactly trace preservation of `N`.) -/
...
trace-preserving. This is precisely the equivalence "the Petz recovery theorem's unitality hypothesis
holds ⟺ `N` is a genuine (trace-preserving) channel", now a theorem in both directions. -/
...
positive reference `σ`, and a strictly positive image `τ`, the Petz map with dual `Ad K` recovers the
reference: `P_{σ, Ad K}(τ) = σ`. Unlike `petzMap_recovery`, the dual here is an *explicit* map, not a
unitality hypothesis. -/
