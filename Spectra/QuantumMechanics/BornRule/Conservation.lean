/-
Copyright (c) 2026 Spectra Library Formalization Project. All rights reserved.
...
**Hypotheses**:
- `h_dirac`: ψ satisfies the Dirac equation (real mass)
- `h_nonzero`: ψ is not the zero state (otherwise probability is undefined) -/
theorem born_rule_valid (Γ : GammaMatrices) (ψ : SpinorField) (t : ℝ) (m : ℝ)
    (_h_dirac : ∀ x, (∑ μ : Fin 4, I • (Γ.gamma μ).mulVec (partialDeriv' μ ψ.ψ x))
      = (↑m : ℂ) • ψ.ψ x)
