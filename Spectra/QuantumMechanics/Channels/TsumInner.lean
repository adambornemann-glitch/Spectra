...
norms, on any inner product space over `RCLike 𝕜`.  These carry no trace-class content — they are the
underlying inner-product facts behind bounds such as `|tr T| ≤ ‖T‖₁`.
...
form is `weighted_norm_tsum_inner_le`; this is the sum-of-`‖·‖`s form, used where a *termwise* absolute
bound is needed.) -/
