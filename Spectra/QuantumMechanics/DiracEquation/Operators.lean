...
/-- An unbounded linear operator with explicit domain (interface type, kept from the old
file; the constructed `generator` is the canonical inhabitant via `toDiracOperator`). -/
...
symmetry is the theorem `generator_isFormalAdjoint`.Full self-adjointness is the deficiency-index
project (`SelfAdjoint.lean`), not a stored assumption.
...
/-- Extract a `DiracOperator` (for results that only need domain + op). -/
noncomputable def toDiracOperator (H_D : DiracHamiltonian H K M) : DiracOperator H where
  domain := generatorDomain H_D.U_grp
  op := (generator H_D.U_grp).toFun
