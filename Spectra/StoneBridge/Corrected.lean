...
The two constructions are packaged as two `Equiv`s of the same type:
`stoneEquivBorel` (built here from `stoneGroup`) and `stoneEquiv` (built in `YosidaHille.Basic`
from `genToGroup`). It is tempting to regard the equality of these `Equiv`s as the theorem. It is
not, and the reason is worth stating precisely, because the easy proof hides it:

* Both `Equiv`s have the **same forward map**, `U ↦ ⟨generator U, _⟩`, definitionally. Hence
  `stoneEquivBorel = stoneEquiv` follows from `Equiv.ext` by `rfl` alone — *without ever
  comparing the two inverse constructions*, since an `Equiv`'s inverse is forced by its forward
  map (two-sided inverses are unique). So `stoneEquivBorel_eq_stoneEquiv` is true but carries
...
The inverse-map agreement read off the `Equiv` equality (`stoneEquivBorel_symm_eq`) therefore
*re-derives* `stoneGroup_eq_genToGroup`; §1 is the conceptually primary statement and §3 is its
formal shadow. We keep both, clearly labeled, rather than let the shadow pose as the substance.
...
* `stoneEquivBorel` — Stone's theorem as a bijection, with the inverse furnished by `stoneGroup`.
* `stoneEquivBorel_eq_stoneEquiv` — the Borel and Yosida *packagings* coincide as `Equiv`s
...
group is determined by its generator (`group_unique`, the uniqueness half of Stone's theorem),
the two groups are equal. No comparison of spectral data is required. -/
...
/-- **Stone's theorem, Cayley–Borel form.** The bijection between strongly continuous
one-parameter unitary groups on `H` and self-adjoint operators on `H`, with inverse given by the
...
(Renamed from `stoneEquivSpectral`: the inverse is built by the *Borel* functional calculus via
the Cayley transform, so `Borel` names the construction and pairs with `Yosida`. "Spectral" was
not wrong — the spectral theorem underlies the Borel calculus — but it was less specific.) -/
noncomputable def stoneEquivBorel [Nontrivial H] :
...
@[simp] lemma stoneEquivBorel_apply [Nontrivial H] (U : OneParameterUnitaryGroup (H := H)) :
    (stoneEquivBorel U : H →ₗ.[ℂ] H) = generator U := rfl

@[simp] lemma stoneEquivBorel_symm_apply [Nontrivial H] (hA : IsSelfAdjoint A) :
    stoneEquivBorel.symm ⟨A, hA⟩ = stoneGroup hA := rfl

@[simp] lemma stoneEquivBorel_symm_U [Nontrivial H] (hA : IsSelfAdjoint A) (t : ℝ) (ψ : H) :
    (stoneEquivBorel.symm ⟨A, hA⟩).U t ψ = stoneExp hA t ψ := rfl

/-! ## §3  The two packagings coincide

We record that the Borel and Yosida `Equiv`s are equal. Read the logical-structure note in the
...
/-- The Borel and Hille–Yosida packagings of Stone's theorem are the same `Equiv`.

**This proof carries no mathematical content about the two constructions.** The forward maps of
`stoneEquivBorel` and `stoneEquiv` are definitionally equal (both `U ↦ ⟨generator U, _⟩`), so
...
theorem stoneEquivBorel_eq_stoneEquiv [Nontrivial H] :
    (stoneEquivBorel : OneParameterUnitaryGroup (H := H) ≃ _) = stoneEquiv :=
  Equiv.ext fun _ => rfl
...
theorem stoneEquivBorel_symm_eq [Nontrivial H] :
    (stoneEquivBorel.symm : {A : H →ₗ.[ℂ] H // IsSelfAdjoint A} → _) = stoneEquiv.symm := by
  rw [stoneEquivBorel_eq_stoneEquiv]
